...
class ChildModel extends Model{
  ...
  Future<void> addChildData(ChildData f) async {
    ...
    await Firestore.instance.collection("users").document(user.firebaseUser.uid)
        .collection("filhos").add(f.toMap()).then((doc){
  ...
  Future<void> removeChildData(ChildData f) async {
    await Firestore.instance.collection("users").document(user.firebaseUser.uid)
        .collection("filhos").document(f.cid).delete();
  ...
  Future<void> _loadChild() async {
    QuerySnapshot query = await Firestore.instance.collection("users").document(user.firebaseUser.uid)
        .collection("filhos").getDocuments();
    listChildren = query.documents.map((doc) => ChildData.fromDocument(doc)).toList();
    notifyListeners();
  }
